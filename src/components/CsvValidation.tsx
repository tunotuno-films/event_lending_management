      ...
      try {
        ...
        const validatedItemsPromises = csvData.map(async (item, index) => {
          ...
          if (item.item_id && /^\d{8}$|^\d{13}$/.test(item.item_id)) {
            try {
                let isDuplicate = false; // 重複チェック用変数を宣言
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex gap-2 self-end sm:self-auto">
            ...
            <button
              ...
              disabled={isProcessing || isValidating || !items.some(item => item.isValid)} // 有効なアイテムがない場合も無効化
              className={`px-4 py-2 rounded-md flex items-center justify-center min-w-[100px] ${ // min-width設定
                isProcessing || isValidating || !items.some(item => item.isValid)
                  ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
            >
              {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : '一括登録'}
            </button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto border border-gray-200 rounded-lg">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">状態</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">物品ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">物品名</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">ジャンル</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/6">管理者</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/6">エラー内容</th>
                </tr>
              </thead>
