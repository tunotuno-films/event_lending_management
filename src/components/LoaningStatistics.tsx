...
// import type { Options as Html2CanvasOptions } from 'html2canvas'; // 型が見つからないためコメントアウト
...
export default function LoaningStatistics() {
  ...
  const displayStatistics = useMemo(() => {
    if (!mergeByName) return statistics;
       ...
       if (record.start_datetime) { // Check if start_datetime exists
        try {
            ...
            if (intervalIndex >= 0 && intervalIndex < totals.length) {
              totals[intervalIndex]++; // Increment count, not duration
            }
          } catch (e) {
  ...
  const topItemsHourlyUsage = useMemo(() => {
    const topN = 5;
    // Use sortedDisplayStatistics to get top items based on loan_count
    ...
    // Return the correct structure including topItems list
  ...
  const getHeatmapColor = (count: number, maxCount: number) => {
    if (count === 0) return { backgroundColor: 'rgb(249, 250, 251)', color: 'rgb(156, 163, 175)' };
  ...
  const chartOptions: ChartOptions<'bar'> = useMemo(() => {
    ...
    switch (chartType) {
      case 'loan_count':
        xAxisTitle = '貸出回数'; chartTitle = '人気車両ランキング (貸出回数)'; break;
      case 'total_duration':
        xAxisTitle = '総貸出時間 (秒)'; chartTitle = '人気車両ランキング (総貸出時間)'; break;
      case 'average_duration':
        xAxisTitle = '平均貸出時間 (秒)'; chartTitle = '人気車両ランキング (平均貸出時間)'; break;
    }
    return {
      ...
      scales: {
        x: {
          beginAtZero: true, title: { display: true, text: xAxisTitle },
          ticks: {
        ...
        y: { title: { display: true, text: '物品名' }, ticks: { autoSkip: false } },
      },
      plugins: {
          ...
          callbacks: {
            label: function(context) {
              ...
              if (context.parsed.x !== null) {
                label += (chartType === 'total_duration' || chartType === 'average_duration')
                  ? formatDuration(context.parsed.x)
                  : context.parsed.x;
              }
              return label;
            }
  ...
  const chartData: ChartData<'bar', number[], string> = useMemo(() => {
    const sortedData = [...sortedDisplayStatistics].sort((a, b) => {
        const aValue = a[chartType]; const bValue = b[chartType];
      ...
      let datasetLabel = '';
      switch (chartType) {
        case 'loan_count': datasetLabel = '貸出回数'; break;
        case 'total_duration': datasetLabel = '総貸出時間'; break;
        case 'average_duration': datasetLabel = '平均貸出時間'; break;
      }
      return {
        labels,
        datasets: [{
          label: datasetLabel, data: dataValues,
  ...
  const lineChartOptions: ChartOptions<'line'> = useMemo(() => {
    ...
    return {
        ...
        scales: {
            x: {
                title: { display: true, text: '時間帯 (10分間隔)' },
                ticks: { autoSkip: true, maxRotation: 90, minRotation: 0 }
            },
            y: {
                beginAtZero: true, suggestedMax: yAxisSuggestedMax,
                title: { display: true, text: '合計貸出回数' },
                ticks: {
                  stepSize: 2, precision: 0,
                  callback: function(value) {
                      const numericValue = typeof value === 'string' ? parseFloat(value) : value;
                      if (numericValue % 2 === 0) return numericValue;
                      return undefined;
                  }
                },
            },
        ...
        plugins: {
            legend: { display: false },
            title: { display: true, text: '時間帯別の貸出傾向 (イベント全体 - 10分間隔)' },
  ...
  const topItemsLineChartOptions: ChartOptions<'line'> = useMemo(() => ({
    ...
    scales: {
        ...
        y: {
            beginAtZero: true, title: { display: true, text: '物品別 貸出回数' },
            ticks: { precision: 0 },
        },
    },
    plugins: {
        legend: { display: true, position: 'top' },
        title: { display: true, text: '人気車両の時間帯別貸出傾向 (上位5件 - 10分間隔)' },
        tooltip: { mode: 'index', intersect: false },
    },
}), []);
  ...
  const topItemsLineChartData: ChartData<'line', number[], string> = useMemo(() => {
    ...
    if (firstIndex === -1 || topItems.length === 0) return { labels: [], datasets: [] };
    ...
    const datasets = topItems.map((item, index) => {
        ...
        return {
            label: item.item_name, data: data,
            borderColor: color, backgroundColor: `${color.slice(0, -1)}, 0.5)`,
            tension: 0.1, fill: false,
        };
    });
    ...
    return {
        ...
        scales: {
            x: {
                ...
                title: { display: true, text: '貸出回数' }, beginAtZero: true,
            },
            y: {
                type: 'linear', title: { display: true, text: yAxisLabel },
                beginAtZero: true,
                ticks: { callback: function(value) { return typeof value === 'number' ? formatDuration(value) : value; } }
            }
        },
        plugins: {
            legend: { display: false },
  ...
  const generateCsvData = (): { data: Blob, filename: string } | null => {
    ...
    const csvData = dataToExport.map(stat => {
      const itemIdToExport = mergeByName ? stat.original_item_ids?.[0] ?? '' : stat.item_id;
      const commonData = [
  ...
  const generateHeatmapImageData = async (): Promise<{ data: string, filename: string } | null> => {
    try {
      const html2canvasModule = await import('html2canvas');
      const html2canvas = html2canvasModule.default;
      const heatmapElement = document.getElementById('heatmap-table-container');
      if (!heatmapElement) throw new Error('Heatmap element not found');
      ...
      const canvas = await html2canvas(heatmapElement, {
        ...
        width: heatmapElement.scrollWidth, height: heatmapElement.scrollHeight,
        letterRendering: true, allowTaint: true, backgroundColor: '#ffffff',
        logging: false,
        onclone: (doc: Document) => { 
            const clonedElement = doc.getElementById('heatmap-table-container');
            if (clonedElement) {
      ...
      } as any ); // Use 'any' for html2canvas options type for now

      if (scrollContainer) scrollContainer.scrollLeft = originalScrollLeft;
      ...
      return { data: dataUrl, filename };
    } catch (error) {
      console.error('Error generating heatmap image:', error);
      setNotification({ show: true, message: 'ヒートマップ画像の生成に失敗しました', type: 'error' });
      return null;
    }
  };

  const generateChartImageData = (chartRef: React.RefObject<ChartJS | null>, baseFilename: string): { data: string, filename: string } | null => {
    if (chartRef.current) {
      ...
      const mergeSuffix = mergeByName ? '_統合' : '';
      const filename = selectedEvent
        ? `${dateString}_${baseFilename}_${selectedEvent.event_id}-${selectedEvent.name}${mergeSuffix}.png`
        : `${dateString}_${baseFilename}${mergeSuffix}.png`;
      return { data: dataUrl, filename };
    } else {
      console.error(`Chart ref for ${baseFilename} is not available.`);
      setNotification({ show: true, message: 'グラフ画像の生成に失敗しました。', type: 'error' });
      return null;
    }
  };
  ...
  const generateBarChartData = () => {
    ...
    return generateChartImageData(chartRef, `人気ランキング_${chartTypeSuffix}`);
  };

  const generateOverallLineChartData = () => {
    return generateChartImageData(lineChartRef, '時間帯別傾向');
  };

  const generateTopItemsLineChartData = () => {
    return generateChartImageData(topItemsLineChartRef, '人気車両時間帯別傾向');
  };
  ...
  const generateScatterChartData = () => {
    ...
    return generateChartImageData(scatterChartRef, chartTypeSuffix);
  };
      ...
      {selectedEventId && (
        <>
          <div className="flex justify-end mb-4">
            <DownloadButton 
              onGenerateData={async () => generateCsvData()}
              idleText="CSVダウンロード"
            />
          </div>
          ...
          <div className="mt-8">
            <div className="mb-4 flex justify-between items-center">
              ...
              <DownloadButton 
                idleText="画像ダウンロード"
                loadingText="生成中..."
                onGenerateData={generateHeatmapImageData}
              />
            </div>
          ...
          <div className="mt-8">
            <div className="mb-4 flex justify-between items-center">
              ...
              <DownloadButton 
                idleText="画像ダウンロード"
                loadingText="生成中..."
                onGenerateData={async () => generateBarChartData()}
              />
            </div>
            ...
            <div className="mt-12">
              <div className="mb-4 flex justify-between items-center">
                ...
                <DownloadButton
                  idleText="画像ダウンロード"
                  loadingText="生成中..."
                  onGenerateData={async () => generateOverallLineChartData()}
                />
              </div>
            ...
            <div className="mt-12">
              <div className="mb-4 flex justify-between items-center">
                ...
                <DownloadButton
                  idleText="画像ダウンロード"
                  loadingText="生成中..."
                  onGenerateData={async () => generateTopItemsLineChartData()}
                />
              </div>
            ...
            <div className="mt-12">
              <div className="mb-4 flex justify-between items-center">
                ...
                <DownloadButton
                  idleText="画像ダウンロード"
                  loadingText="生成中..."
                  onGenerateData={async () => generateScatterChartData()}
                />
              </div>
